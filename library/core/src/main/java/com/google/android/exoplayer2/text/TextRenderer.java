...
public class TextRenderer extends BaseRenderer implements Callback {
  ...
  protected void onStreamChanged(Format[] formats, long startPositionUs, long offsetUs) {
    boolean formatChanged = !formats[0].equals(streamFormat);
    streamFormat = formats[0];
    if (decoder != null) {
      if(formatChanged) decoderReplacementState = REPLACEMENT_STATE_SIGNAL_END_OF_STREAM;
    } else {
      initDecoder();
    }
    renderCount = 0;
  ...
  public void render(long positionUs, long elapsedRealtimeUs) {
    ...
    try {
      while (!inputStreamEnded) {
        ...
        @SampleStream.ReadDataResult int result = readTextSource(formatHolder, nextInputBuffer, false);
